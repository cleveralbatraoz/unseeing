#!/bin/sh
# Editor-mode slab probe — the gate on the one branch a headless test suite
# cannot reach.
#
# WaveLevel hides its ceiling when Engine.is_editor_hint() is true, so a map
# can be laid out from directly above instead of through a 28 x 28 m opaque
# lid. Godot exposes is_editor_hint() but NO setter — not to GDScript, not
# in the gdext bindings — so every gdUnit4 run is in run mode, and cutting
# the line that applies the law left the whole suite green.
#
...
PROBE="res://tests/probe/editor_slab_probe.gd"

# The probe reaches the level through the Rust extension, and the engine
# loads that only when .godot/extension_list.cfg names it — a generated file
# git deliberately does not track. A fresh clone therefore has no engine
# classes at all, and a probe that found no WaveLevel would be reporting on
# an empty engine rather than on the level. Import first; the probe still
# refuses loudly if the class is missing afterwards.
...
run_mode() {
  ...
  if ! printf '%s' "$out" | grep -q "^# slabs: mode=$want\$"; then
