...
# gitignored — see CLAUDE.md's godot-mcp paragraph: a committed addon would
# ship to the droplet and the wasm export via deploy.sh's `git archive`, and
# would break ci/vendor-gdunit4.sh's assumption that gdUnit4 is the only
# tenant of game/addons/). That reference would NOT be caught by this
# project's CI boot gate: [editor_plugins] only loads in the EDITOR, so the
# headless `--quit-after` boot check (no `-e`) produces no output at all for
# a missing addon, and even an editor-mode run only WARNS ("Addon ... failed
